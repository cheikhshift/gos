			...
			if imp.Type == "f" {
				...
				if !strings.Contains(handlerInternalTests, tlc) {
					...
					func Test%s(t *testing.T){
						...
						t.Error("Test for %s not implemented!!")
					}`, tlc, tlc)
				}
    ...
    `
		...
		*/
		...
		for _, imp := range template.Header.Structs {
			...
			// Create a new %s struct.
			func Netstruct%s() *%s{ return &%s{} }`, imp.Name,imp.Name,imp.Name, imp.Name, imp.Name,imp.Name, imp.Name, imp.Name)
