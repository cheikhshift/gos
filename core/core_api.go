...
`
		...
		`
		if _, err := os.Stat("./api/handlers/handlers_internal_test.go"); !os.IsNotExist(err) {

			data, _ := ioutil.ReadFile("./api/handlers/handlers_internal_test.go")
			handlerInternalTests = string(data)
		...
		`
		if _, err := os.Stat("./api/handlers/handlers_test.go"); !os.IsNotExist(err) {

			data, _ := ioutil.ReadFile("./api/handlers/handlers_test.go")
			handlerTests = string(data)
		...
		ioutil.WriteFile("./api/handlers/handlers_internal_test.go", []byte(handlerInternalTests), 0700)
		...
		for _, imp := range template.Endpoints.Endpoints {
			...
			if imp.Type != "f" {
				if !strings.Contains(handlerTests, tlc) {
					handlerTests += fmt.Sprintf(`
					func Test%s(t *testing.T){
		...
		ioutil.WriteFile("./api/handlers/handlers_test.go", []byte(handlerTests), 0700)
    ...
    `
		...
		*/
		...
		`
		if _, err := os.Stat("./types/types_test.go"); !os.IsNotExist(err) {

			data, _ := ioutil.ReadFile("./types/types_test.go")
			structTests = string(data)
		...
		for _, imp := range template.Header.Structs {
			...
			// %s. This function is used with template functionality.
			func  Cast%s(args ...interface{}) *%s  {
			...
			// Create a new %s struct and return the pointer.
			func New%s() *%s{ return &%s{} }`, imp.Name,imp.Name,imp.Name, imp.Name, imp.Name,imp.Name, imp.Name, imp.Name)



			netMa += fmt.Sprintf(`,"%s" : types.New%s`, imp.Name, imp.Name)
			netMa += fmt.Sprintf(`,"is%s" : types.Cast%s`, imp.Name, imp.Name)

			if !strings.Contains(structTests, imp.Name) {
					structTests += fmt.Sprintf(`
					func Test%s(t *testing.T){
		...
		ioutil.WriteFile("./types/types_test.go", []byte(structTests), 0700)
		...
		`
		if _, err := os.Stat("./api/globals/globals_test.go"); !os.IsNotExist(err) {

			data, _ := ioutil.ReadFile("./api/globals/globals_test.go")
			globalTests = string(data)
		...
		for _, imp := range template.Variables {
			...
			if !strings.Contains(globalTests,globalName) {
					...
					globalTests += fmt.Sprintf(`
					func Test%s(t *testing.T){

						t.Error("Test for global variable %s not implemented!!")
					}
		...
		ioutil.WriteFile("./api/globals/globals_test.go", []byte(globalTests), 0700)
		...
		`
		if _, err := os.Stat("./api/methods/methods_test.go"); !os.IsNotExist(err) {

			data, _ := ioutil.ReadFile("./api/methods/methods_test.go")
			methodTests = string(data)
		...
		for _, imp := range available_methods {
			if !contains(int_methods, imp) && !contains(api_methods, imp) {
				...
				if template.Type == "package" {
					...
					if !strings.Contains(methodTests, meth.Name) {
						methodTests += fmt.Sprintf(`
						func Test%s(t *testing.T){
		...
		ioutil.WriteFile("./api/methods/methods_test.go", []byte(methodTests), 0700)
		...
		`
		if _, err := os.Stat("./api/templates/templates_test.go"); !os.IsNotExist(err) {

			data, _ := ioutil.ReadFile("./api/templates/templates_test.go")
			templateTests = string(data)
		...
		for _, imp := range template.Templates.Templates {
			...
			templateString := fmt.Sprintf(`
				...
				func (pkg PKG) %s(args ...interface{}) string {
					...
					result = templates.%s(data)

    				return result
				}`, commentstring, imp.Name, imp.Struct, imp.Struct, imp.Name)
		...
		renderFile := fmt.Sprintf(`
			...
			func RenderTemplate(w http.ResponseWriter, p *gosweb.Page%s   {
				...
				}`, TraceinFunc, web, web, template.ErrorPage, TraceParam, template.ErrorPage, TraceTemplate, web, template.ErrorPage, TraceParam )
